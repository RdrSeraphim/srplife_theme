    ...
    for (const child of postContent.children) {
        ...
        if (
            ...
            child.nextElementSibling.classList.contains(
                notesWrapperSelector.slice(1)
            )
        ) {
            continue;
        }

        const footnoteLinks = child.querySelectorAll(footnoteRefSelector);

        if (footnoteLinks.length) {
            const sidenoteContainer = document.createElement("div");
            sidenoteContainer.setAttribute(
                "class",
                notesWrapperSelector.slice(1)
            );

            for (let footnoteLink of footnoteLinks) {
                const parentWithId = footnoteLink.parentElement;
                const id = parentWithId.id;
                // Ghost IDs are usually fnref1, fn1.
                // The anchor href points to #fn:1. The anchor id is fnref1.
                // The content id is fn1.
                const contentId = footnoteLink
                    .getAttribute("href")
                    .replace("#", "");
                ...
                sidenoteWrapper.setAttribute("data-anchor-id", parentWithId.id); // Use the anchor's ID to find position
                ...
                const links =
                    sidenoteWrapper.querySelectorAll("a.footnote-backref");
                ...
                sidenoteWrapper.insertAdjacentHTML(
                    "afterbegin",
                    `<div class='note-identifier'>${footnoteLink.textContent}</div>`
                );
                sidenoteContainer.insertAdjacentElement(
                    "beforeend",
                    sidenoteWrapper
                );
            }
...
function onResize() {
    const sidenotesInDom = Boolean(
        document.querySelector(notesWrapperSelector)
    );
    const mediaQuery = window.matchMedia("(min-width: 102rem)");
    ...
    if (document.querySelector(postSelector)) {
        if (showFootnotes) {
            ...
            document.addEventListener("click", (evt) => {
                ...
                if (
                    !evt.target.closest(".note") &&
                    !evt.target.closest(".footnote-ref")
                ) {
                    dehilightNotes();
                }
            });
